...
const CustomerTable: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      setLoading(true);
      try {
        const response: PaginatedUserResponse = await getAllUsers(pageIndex, 10, searchTerm);
    ...
    fetchUsers();
  }, [searchTerm, pageIndex]); // Fetch users whenever searchTerm or pageIndex changes
    ...
    <NavbarVertical>
      <div className="profile-page">
        <div className="d-flex justify-content-between align-items-center my-3">
          ...
          />
          <a href="/customer/add-customer" className="btn btn-success">
            Thêm mới
          </a>
        </div>
        {loading ? (
